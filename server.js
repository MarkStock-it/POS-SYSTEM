...
const { loadSeedProducts, seedDatabase } = require('./scripts/seed');
  ...
  db.serialize(() => {
    ...
    ensureColumns(async (ensureErr) => {
      ...
      try {
        ...
        await seedDatabase(db, products);
      } catch (seedErr) {
        console.error('Failed to seed products:', seedErr.message);
      }
    });
  });
}
